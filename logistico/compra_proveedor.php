...
$(document).ready(function(){
    ...
    $('#id_proveedor').change(function(){
        ...
        $.getJSON('compra_proveedor_productos.php', { proveedor_id: provId }, function(res){
            ...
            let html = '';
            prods.forEach(p=>{
                const idx = p.id_catalogo;
                if (!idx) {
                    console.warn('Producto sin id_catalogo:', p);
                    return;
                }
                ...
                <div class="row g-2 mb-2">
                    <div class="col-md-6">
                        ...
                        <input type="hidden" name="id_catalogo[]" value="${idx}">
                        <input type="hidden" name="precio[]" value="${p.precio_compra}">
                    </div>
                    <div class="col-md-3">
   ...
   // Manejador de submit con fetch - CORREGIDO FINAL
$('#formNuevaCompra').on('submit', function(e) {
    ...
    $('input[name="id_catalogo[]"]').each(function(index) {
        const idCatalogo = $(this).val();
        const precio = $('input[name="precio[]"]').eq(index).val();
        const cantidad = $('input[name="cantidad[]"]').eq(index).val();
        
        datos.productos[idCatalogo] = {
            precio: precio,
            cantidad: cantidad
        };
    });
